...
(function(){
  ...
  V.Kalvosto = { start(ctx){
    ...
    const fnrs = fnrMis.map((mi, k) => { const c = mInfo[mi], fad = ofType(mi, 10), ps = psis[0], sinkPlace = ps ? ps.fdPlace : 21;
      ...
      { const fdR = fdMis.length ? hullR(fdMis[0]) : 16, up = hullR(mi) + fdR + 4;   // the dock clear of the two hulls (the ferredoxin's real hull, 22, not the 2D radius 16)
        const s = placeSlot(fnr, 'ferredoxin', mi, [c.cx, c.cy + up, c.cz], LF); s.place_in_the_chain = sinkPlace - 1;
    ...
    window.gValoSpawnProtons = (n, sideOnly) => { const P = get.gValoProtons(), lo = get.gPsuLo(), hi = get.gPsuHi(); if(!P || !lo) return 0; const hT = get.gMemHalfT(), z = get.gTasoZ(); let made = 0;   // sideOnly: 1 = all into the stroma, -1 = all into the lumen, else alternating (owner 14.9.2026: separate buttons)
      ...
      for(let k=0;k<n;k++){ if(!reserveP.length) break; const i = reserveP.pop(), side = sideOnly ? Math.sign(sideOnly) : (k % 2 === 0 ? 1 : -1), x = Math.max(lo.x + 40, Math.min(hi.x - 40, tx + (Math.random()-0.5)*300)), my = P.memY(x, z);
    ...
    scene.onBeforeRenderObservable.add(() => {
      ...
      { const P = get.gValoProtons(); if(P){
        if(!protonsHomed && get.gPsuLo()){ protonsHomed = true; const bx = P.box(), hT = get.gMemHalfT();   // 100 protons, 50 in the stroma and 50 in the lumen, inside the box (owner 13.9.2026)
          ...
          P.homeFlush(); }
        for(const sl of V.all){ if(sl.alive && sl.parent_is_BindSites && sl._proton != null){ if(sl.modulate === 'dark'){ V.env.slotEmptied(sl); continue; }   // (a DARK slot still holding a proton index: the proton is let go where the slot is - it would stay pinned there for good, 15.9.2026)
          const q = sl.global_position; P.place(sl._proton, q[0], q[1], q[2]); } } } }   // a bound proton rides its slot
      ...
      for(const [b, kin] of kinOf){ if(kin.kind !== 'bnc' || !kin.b) continue; if(!b.alive && kin.body !== b && !kin.home){ kinOf.delete(b); continue; }   // (a superseded body: every re-instantiation added one)
        ...
        if(kin.sx == null || Math.hypot(x - kin.sx, y - kin.sy) > 4){ kin.sx = x; kin.sy = y; kin.stillT = 0; } else if((kin.stillT = (kin.stillT || 0) + dt) > 2 && !kin.home && !kin.detour){ kin.stuckT = 4; }   // (owner 15.9.2026: a ferredoxin pinned against PSI's shell for 30 s - its target, a homing FNR, kept coming closer, so 'headway' never ran out; not having MOVED for 2 s counts too)
        ...
        const spE = kin.home ? sp*Math.min(1, d/30) : sp;   // a HOMING protein slows down over the last 30 units and rests at home (it kept full speed and circled its home for ever - the 'restless FNR', owner 15.9.2026)
        const k = (b.alive && b.hard_target) ? 0.5 : 0.06, f = 1 - Math.pow(1-k, dt*60); const vx = bb.vx + (dx/d*spE - bb.vx)*f, vy = bb.vy + (dy/d*spE - bb.vy)*f, l = Math.hypot(vx, vy) || 1; bb.vx = vx/l*spE; bb.vy = vy/l*spE; }
      ...
      heats.length = 0; showGlow(heat, 0);   // (no glow spheres of any kind - owner 13.9.2026: 'still there is bubbles, dont make them')
        flashes.length = 0; showGlow(flashG, 0);
        rings.length = 0; showGlow(ringG, 0);
      tickLabels();
