...
Kalvojen kaukotaso (slab / cellshell) piirtaa omistajan pilkkutekstuurin proseduraalisen rakeen sijaan: `simulaatiot/laatta_vihrea.jpg` viherhiukkasen kalvoille ja tylakoideille (nimi /viherhiukkanen|tylakoidi/) seka kalvostotilan levylle, `laatta_keltainen.jpg` kaikille muille. Shaderin `laatta()` olettaa noin 180 pilkkua laatan sivulla (yksi pilkku = yksi lipidi, laatta = 360 x grainPitch) ja tuplaa laatan monitasoisesti kun pilkut kutistuvat pikseliksi. Nykyiset kuvat ovat `tyokalut/laatta_tee.py`:n generoimia saumattomia varakuvia - omistajan omat ruutukaappaukset saumattomaksi ja oikean kokoiseksi: `python tyokalut/laatta_tee.py kuva.png simulaatiot/laatta_vihrea.jpg`. Puuttuva tiedosto -> texOn 0 ja vanha rae. Muista `git add` (muuten 404 solukko.comissa).
...
- Laatan vari (12.9.2026): laatta (levyn memSlab ja solun cellshell) on AINA samanvarinen kuin lahilipidien vdW-pallot: slabCol = lipidien oma keskivari tintteineen, ja laatta_*.jpg toimii vain kirkkauskuviona (luminanssi / kuvan keskiluminanssi texMean, luetaan kerran readPixelsilla latauksessa, kerrottuna rakeen keskisavylla 0.80). Kuvan omat varit eivat enaa nay - keltainen ja vihrea laatta eroavat vain kuvioltaan.
