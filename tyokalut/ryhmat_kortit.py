...
3D-malli-kentassa on ryhman nimi ja nelja yhdistetta 'ryhma: AVAIN1, AVAIN2, AVAIN3, AVAIN4' (ryhmat_3d.KORTIT_MALLIT,
omistaja 14.9.2026): sivusto nayttaa vastauspuolella 2x2-ruudukon 2D-kaavoja / elavia 3D-malleja, joissa ryhman atomit on
korostettu vihrealla aariviivalla (molekyylit.json: ryhmat, kuva2d_ryhmat); popupissa vain ensimmainen. Laaja vastaus
mainitsee ensimmaisen molekyylin nimelta, joten sana avaa popupin.
...
import ryhmat_3d
from ryhmat_3d import KORTIT_MALLIT   # 14.9.2026: nelja yhdistetta / ryhma, 3D-malli-kentta 'ryhma: AVAIN1, AVAIN2, AVAIN3, AVAIN4'
...
K("Mikä on metyyliryhmä?",
  "Metyyliryhmä on yhden hiilen hiilivetyryhmä –CH<sub>3</sub>. Hiileen on sitoutunut kolme vetyä.",
...
K("Mikä on etyyliryhmä?",
  "Etyyliryhmä on kahden hiilen hiilivetyryhmä –CH<sub>2</sub>CH<sub>3</sub>.",
...
K("Mikä on fenyyliryhmä?",
  "Fenyyliryhmä on bentseenirengas substituenttina. Siinä on kuusi hiiltä tasomaisessa aromaattisessa renkaassa (–C<sub>6</sub>H<sub>5</sub>).",
...
K("Mikä on karbonyyliryhmä?",
  "Karbonyyliryhmä on hiili, johon happi on sitoutunut kaksoissidoksella (C=O).",
...
K("Mikä on aldehydiryhmä?",
  "Aldehydiryhmä on karbonyyli ketjun päässä. Hiileen on sitoutunut happi kaksoissidoksella ja yksi vety (–CHO).",
...
K("Mikä on ketoniryhmä?",
  "Ketoniryhmä on karbonyyli ketjun keskellä. Hiileen on sitoutunut happi ja kaksi hiiltä (R–CO–R').",
...
K("Mikä on karboksyyliryhmä?",
  "Karboksyyliryhmä on happoryhmä –COOH, joka luovuttaa protonin. Solussa se on yleensä karboksylaattina –COO<sup>−</sup>.",
...
K("Mikä on hydroksyyliryhmä?",
  "Hydroksyyliryhmä on alkoholiryhmä –OH. Hiileen on sitoutunut happi, jossa on vety.",
...
K("Mikä on enoliryhmä?",
  "Enoliryhmä on hydroksyyli, joka on kiinni hiili–hiili-kaksoissidoksen hiilessä (C=C–OH).",
...
K("Mikä on eetteriryhmä?",
  "Eetteriryhmä on happi kahden hiilen välissä (R–O–R').",
...
K("Mikä on esteriryhmä?",
  "Esteriryhmä on karboksyylihapon ja alkoholin liitos. Karbonyylihiili on sitoutunut happeen, joka jatkuu hiileen (R–CO–O–R').",
...
K("Mikä on asetyyliryhmä?",
  "Asetyyliryhmä on etikkahaposta peräisin oleva kahden hiilen ryhmä –CO–CH<sub>3</sub>.",
...
K("Mikä on happoanhydridi?",
  "Happoanhydridi on kaksi karboksyylihappoa, jotka ovat liittyneet hapen kautta vettä menettäen (R–CO–O–CO–R').",
...
K("Mikä on aminoryhmä?",
  "Aminoryhmä on typpi kahdella vedyllä (–NH<sub>2</sub>). Emäksenä se sitoo protonin muotoon –NH<sub>3</sub><sup>+</sup>.",
...
K("Mikä on amidiryhmä?",
  "Amidiryhmä on karbonyyli, jonka hiileen on sitoutunut typpi (R–CO–NH<sub>2</sub>).",
...
K("Mikä on imiiniryhmä?",
  "Imiiniryhmä on hiili–typpi-kaksoissidos (C=NH). Se on karbonyylin typpivastine.",
...
K("Mikä on Schiffin emäs?",
  "Schiffin emäs on imiini, jonka typpeen on sitoutunut hiiliryhmä (R<sup>1</sup>–C(=N–R<sup>3</sup>)–R<sup>2</sup>).",
...
K("Mikä on guanidiiniryhmä?",
  "Guanidiiniryhmä on hiili, johon on sitoutunut kolme typpeä. Vahvana emäksenä se on solussa positiivinen guanidinium.",
...
K("Mikä on imidatsoliryhmä?",
  "Imidatsoliryhmä on viisirengas, jossa on kaksi typpeä. Se on histidiinin sivuketju, pK<sub>a</sub> lähellä 7.",
...
K("Mikä on sulfhydryyliryhmä?",
  "Sulfhydryyliryhmä on tioliryhmä –SH. Hiileen on sitoutunut rikki, jossa on vety.",
...
K("Mikä on disulfidisidos?",
  "Disulfidisidos on kahden rikin välinen kovalenttinen sidos (R–S–S–R'). Se syntyy, kun kaksi tiolia hapettuu.",
...
K("Mikä on tioesteri?",
  "Tioesteri on esteri, jossa alkoholin hapen tilalla on rikki (R–CO–S–R'). Sidos on energiarikas.",
...
K("Mikä on fosforyyliryhmä?",
  "Fosforyyliryhmä on fosfaattiryhmä –OPO<sub>3</sub><sup>2−</sup>, joka on sitoutunut hiileen hapen kautta.",
...
K("Mikä on fosfoanhydridisidos?",
  "Fosfoanhydridisidos on kahden fosfaattiryhmän välinen sidos (P–O–P). Sen vesikatkaisu vapauttaa paljon energiaa.",
...
K("Mikä on asyylifosfaatti?",
  "Asyylifosfaatti on karboksyylihapon ja fosforihapon sekoitettu anhydridi (R–CO–O–PO<sub>3</sub><sup>2−</sup>).",
...
if __name__ == '__main__':
    ...
    for i, k in enumerate(KORTIT):
        ...
        ryhma = re.sub(r'^Mikä on (.+)\?$', r'\1', k[0]); assert ryhma in KORTIT_MALLIT, ryhma
        malli = ryhma + ': ' + ', '.join(KORTIT_MALLIT[ryhma])   # sivusto: 2x2-ruudukko, ryhman atomit vihrealla; popupissa vain ensimmainen
        fields = ['(4.%03d) %s' % (i + 1, k[0]), k[1], k[2], k[3], malli, k[5]]
