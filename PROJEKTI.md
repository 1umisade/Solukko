...
| `simulaatiot/kortit/` | 26 molekyylin ja atomin pyörähdysvideot (`<laji>.webm`, 256 px, ~700 kB, 280 ruutua per kierros) simulaattorin korttirenderistä vdW-pintoineen + `2d/` (molekyylien rakennekaavat `tyokalut/kaava2d.py`:llä sarjakuvatyylissä — punainen vdW-hehku, värilliset atomipallot, kaikki vedyt — ja atomien kuorimallit, kaikki SVG:nä) + `molekyylit.json` (laji → kaavan kirjoitusasut, suomenkieliset sijamuodot, video, kuva2d). **Tiedostonimet eivät saa erota vain kirjainkoolla** (Windows: `p` ja `P` ovat sama tiedosto) — protoni on `Hplus`, fosfori `P`. |
