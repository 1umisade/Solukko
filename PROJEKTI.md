...
| `simulaatiot/` | **Geneesi**, Babylon.js-pohjainen 3D-molekyylisimulaattori (ent. geneesi.com), tuotu `git subtree`lla 10.9.2026 historia mukanaan. Oma `index.html` (~13 000 riviä) + mol2-mallit (~58 MB). Julkinen osoite `solukko.com/simulaatiot/`, etusivun *simulaatiot*-välilehti linkittää sen kolmeen tilaan (`?tila=editori` Kemiaeditori, `kalvosto` Yhteyttämiskalvosto, `solu` **Webcyte** — kokonainen solu 3D:nä; nimi on omistajan valinta 10.9.2026). Perehdytys: `simulaatiot/AGENT_NOTES.md`. Kehitys tapahtuu nyt tässä repossa, ei enää Geneesi-repossa. |
| `simulaatiot/kortit/` | `molekyylit.json` (laji → kaavan kirjoitusasut, suomenkieliset sijamuodot, kuva2d) + `2d/` (molekyylien rakennekaavat `tyokalut/kaava2d.py`:llä sarjakuvatyylissä ja atomien kuorimallit, SVG). Popupin 3D on **elävä simulaattori** iframessa: `simulaatiot/index.html?tila=kortti&laji=<avain>` — yksi molekyyli origossa, vasen veto kääntää, rulla zoomaa, ei panorointia. **Tiedostonimet eivät saa erota vain kirjainkoolla** (Windows): protoni on `Hplus`, fosfori `P`. |
...
- **Molekyylipopup:** kaava (`H<sub>2</sub>O`, `H₂O`, `ATP`, `Pᵢ`…) tai suomenkielinen nimi (vesi, vedessä,
  protoni…) korttitekstissä avaa hoverilla elävän 3D-mallin (simulaattori iframessa, `?tila=kortti`). Jos sanalla on
